...
class DDPM(pl.LightningModule):
    ...
    def predict_start_from_noise(self, x_t, t, noise):
        ## derived from q(x_t | x_0) = sqrt_alphas_cumprod * x0 + sqrt(( 1 - alphas_cumprod)) * noise
        return (
    ...
    def q_sample(self, x_start, t, noise=None):
        ...
        return (extract_into_tensor(self.sqrt_alphas_cumprod, t, x_start.shape) * x_start +
                extract_into_tensor(self.sqrt_one_minus_alphas_cumprod, t, x_start.shape) * noise) ## directly q(x_t | x_0)
...
class LatentDiffusion(DDPM):
    ...
    def encode_first_stage(self, x):
        ## could be optimized by Tiled VAE
        if hasattr(self, "split_input_params"):
            if self.split_input_params["patch_distributed_vq"]:
                ks = self.split_input_params["ks"]  # eg. (128, 128)
                stride = self.split_input_params["stride"]  # eg. (64, 64)
...
class LatentDiffusionSRTextWT(DDPM):
    ...
    def p_mean_variance_canvas_accelerate(self, x, c, struct_cond, t, clip_denoised: bool, return_codebook_ids=False, quantize_denoised=False,
                        return_x0=False, score_corrector=None, corrector_kwargs=None, t_replace=None,
                        tile_size=64, tile_overlap=32, batch_size=4, tile_weights=None,
                        noise_reference=None, offset_x=0, offset_y=0,
                        noise_ema=None, random_offset=False,
                                          random_reset_step=4):
        """
        Aggregation Sampling strategy for arbitrary-size image super-resolution
        accelerated with offset strategy
        and noise reference

        improved strategy:
        1. 划分tile的时候没有overlap, 而是根据现在的offset进行调整
        2. 如果有noise reference, 那么在contribution=0的position, 直接copy noise reference
            2.1 可以进一步引入ema策略 (TODO: 验证有效性)
        ...
        while cur_x < x.size(-1):
            ## [64, 128, ...]
            cur_x = max(grid_rows * tile_size, 0) + tile_size
            grid_rows += 1

        grid_cols = 0
        cur_y = 0
        while cur_y < x.size(-2):
            cur_y = max(grid_cols * tile_size, 0) + tile_size
            grid_cols += 1
        # print("grid rows: {} | grid cols: {}".format(grid_rows, grid_cols))
        if not hasattr(self, "random_offset_step_count"):
            self.random_offset_step_count = 0
        if random_offset:
            # print("use random offset")
            offset_x = random.randint(0, tile_overlap-1)
            offset_y = random.randint(0, tile_overlap-1)
            if (self.random_offset_step_count % random_reset_step) == 0:
                offset_x, offset_y = 0, 0
                # print("reset random step to 0")
            self.random_offset_step_count = (self.random_offset_step_count + 1) % random_reset_step
        ...
        for row in range(grid_rows):
            ...
            for col in range(grid_cols):
                # if col < grid_cols-1 or row < grid_rows-1:
                #     # extract tile from input image
                ofs_x = max(row * tile_size + offset_x, 0)
                ofs_y = max(col * tile_size + offset_y, 0)
                    # input tile area on total image
                ofs_x = min(ofs_x, w - tile_size)
                ofs_y = min(ofs_y, h - tile_size)
                # if row == grid_rows-1:
                #     ofs_x = w - tile_size
                # if col == grid_cols-1:
                #     ofs_y = h - tile_size
                ...
                if len(input_list) == batch_size or col == grid_cols-1:
                    ...
                    if torch.isnan(model_out).any():
                        ...
                        print("there is nan in model out. ")
                        print("x_start: {} | x_end: {} | y_start: {} | y_end: {}".format(input_start_x, input_end_x, input_start_y, input_end_y))
                        exit(0)
                    for sample_i in range(model_out.size(0)):
                        noise_preds_row.append(model_out[sample_i].unsqueeze(0))
                    input_list = []
        ...
        # Stitch noise predictions for all tiles
        noise_pred = torch.zeros(x.shape, device=x.device) ## TODO: maybe add trick here
        contributors = torch.zeros(x.shape, device=x.device) ## strategy from mixture of diffusers. If for multidiffusion, it is 1.0 for each pixel in the tile
        # Add each tile contribution to overall latents
        for row in range(grid_rows):
            for col in range(grid_cols):
                ofs_x = max(row * tile_size + offset_x, 0)
                ...
                ofs_x = min(ofs_x, w - tile_size)
                ofs_y = min(ofs_y, h - tile_size)
                # print(f"ofs x: {ofs_x} | ofs y: {ofs_y}")
                input_start_x = ofs_x
                input_end_x = ofs_x + tile_size
                input_start_y = ofs_y
                input_end_y = ofs_y + tile_size
                ...
                contributors[:, :, input_start_y:input_end_y, input_start_x:input_end_x] += tile_weights
        # Average overlapping areas with more than 1 contributor
        # print("contributers mean: ", contributors.mean())
        # print("contributer zero count: ", (contributors==0).sum())
        # print("contributer zero ratio: ", (contributors==0).sum() / contributors.numel())
        ### save contributors, current offset (check if it is correct)



        # noise_pred /= contributors
        ## the reason of nan: /0
        noise_pred = torch.where(contributors>0, noise_pred / contributors, noise_pred)
        # Use reference on contributors==0
        if noise_reference is not None:
            # print("noise mean before using reference: {}".format(noise_pred[noise_pred!=0].mean()))
            # print("use noise reference")
            noise_pred = torch.where(contributors > 0, noise_pred, noise_reference)
            # print("noise mean after using reference: {}".format(noise_pred.mean()))
            # print("noise zero count after reference: {}".format((noise_pred==0).sum()))
            # noise_pred[contributors==0] = noise_reference[contributors==0]
            if noise_ema is not None:
                ## momentum update noise prediction ##
                print("noise ema enabled")
                noise_pred = noise_ema * noise_reference + (1 - noise_ema) * noise_pred
        # noise_pred /= torch.sqrt(contributors)
        model_out = noise_pred
    ...
    @torch.no_grad()
    def p_sample_canvas_accelerate(self, x, c, struct_cond, t, clip_denoised=False, repeat_noise=False,
                 return_codebook_ids=False, quantize_denoised=False, return_x0=False,
                 temperature=1., noise_dropout=0., score_corrector=None, corrector_kwargs=None, t_replace=None,
                 tile_size=64, tile_overlap=32, batch_size=4, tile_weights=None,
                 noise_reference=None, offset_x=0, offset_y=0,
        ...
        ## get offset x and offset y by state

        outputs = self.p_mean_variance_canvas_accelerate(x=x, c=c, struct_cond=struct_cond, t=t, clip_denoised=clip_denoised,
                                       return_codebook_ids=return_codebook_ids,
                                       quantize_denoised=quantize_denoised,
                                       return_x0=return_x0,
                                       score_corrector=score_corrector, corrector_kwargs=corrector_kwargs, t_replace=t_replace,
                                       tile_size=tile_size, tile_overlap=tile_overlap, batch_size=batch_size, tile_weights=tile_weights,
                                       noise_reference=noise_reference, offset_x=offset_x, offset_y=offset_y,
                                       noise_ema=noise_ema,
                                       random_offset=random_offset,
                                       random_reset_step=random_reset_step)
        ...
        if return_codebook_ids:
            ...
            model_mean, _, model_log_variance, noise_pred = outputs
        ...
        if return_x0:
            ...
            return model_mean + nonzero_mask * (0.5 * model_log_variance).exp() * noise, noise_pred
    ...
    def _gaussian_weights(self, tile_width, tile_height, nbatches):
        ...
        weights = np.outer(y_probs, x_probs) ## the average of weights is nearly 1
    ...
    @torch.no_grad()
    def p_sample_loop_canvas_accelerate(self, cond, struct_cond, shape, return_intermediates=False,
                      x_T=None, verbose=True, callback=None, timesteps=None, quantize_denoised=False,
                      mask=None, x0=None, img_callback=None, start_T=None,
                      log_every_t=None, time_replace=None, adain_fea=None, interfea_path=None,
                      tile_size=64, tile_overlap=32, batch_size=4,
                      offset_stride=32, use_noise_reference=True,
                      complete_offset=True, noise_ema=None,
                      random_offset=False, random_reset_step=4,
                      ):
        ...
        ## generate offset states
        offsets = []
        if complete_offset:
            for offset_x in range(-tile_size, tile_size, offset_stride):
                for offset_y in range(-tile_size, tile_size, offset_stride):
                    offsets.append((offset_x, offset_y))
        else:
            for offset_x in range(0, tile_size, offset_stride):
                for offset_y in range(0, tile_size, offset_stride):
                    offsets.append((offset_x, offset_y))
        offset_state = 0
        noise_reference = None
        for i in iterator:
            ## set offset
            offset_x, offset_y = offsets[offset_state]
            offset_state = (offset_state + 1) % len(offsets)
            # print(f"offset x is {offset_x}, offset y ix {offset_y}")
            ...
            img, noise_pred = self.p_sample_canvas_accelerate(img, cond, struct_cond, ts,
                                clip_denoised=self.clip_denoised,
                                quantize_denoised=quantize_denoised, t_replace=t_replace,
                                tile_size=tile_size, tile_overlap=tile_overlap, batch_size=batch_size, tile_weights=tile_weights,
                                ...
                                random_offset=random_offset,
                                random_reset_step=random_reset_step)
            if use_noise_reference: ## update noise reference as noise pred in this step
                noise_reference = noise_pred
    ...
    ## added by us
    @torch.no_grad()
    def sample_canvas_accelerate(self, cond, struct_cond, batch_size=16, return_intermediates=False, x_T=None,
               verbose=True, timesteps=None, quantize_denoised=False,
               ...
               use_noise_reference=True, offset_stride=32, complete_offset=True,
               noise_ema=None, random_offset=False,
               random_reset_step=4, **kwargs):
        ...
        if cond is not None:
            if isinstance(cond, dict):
                ...
                cond = [c for c in cond] if isinstance(cond, list) else cond
        return self.p_sample_loop_canvas_accelerate(cond,
                                  struct_cond,
                                  shape,
                                  return_intermediates=return_intermediates, x_T=x_T,
                                  verbose=verbose, timesteps=timesteps, quantize_denoised=quantize_denoised,
                                  mask=mask, x0=x0, time_replace=time_replace, adain_fea=adain_fea, interfea_path=interfea_path, tile_size=tile_size,
                                  tile_overlap=tile_overlap, batch_size=batch_size_sample, log_every_t=log_every_t,
                                  use_noise_reference=use_noise_reference, offset_stride=offset_stride,
                                  complete_offset=complete_offset, noise_ema=noise_ema,
                                  random_offset=random_offset, random_reset_step=random_reset_step)
